...
function join(a, ...others) {
  return [...a].concat(
    others.reduce((addOn, other) => {
      return [...addOn, ...other.slice(1, other.length - 1), a[0]];
    }, [])
  );
}
...
function figure8(size, filename) {
  return join(simpleCircle(size, filename), simpleCircle(size, filename));
}
...
const randomLetter10 = () =>
  "abcdefghij".split("")[Math.floor(Math.random() * 10)];
const detected = [];

for (let i = 0; i < 10; i++) {
  detected.push(simpleCircle(3, makeFilename(randomLetter10, uniqueLetter)));
}
for (let i = 0; i < 7; i++) {
  detected.push(simpleCircle(2, makeFilename(randomLetter10, uniqueLetter)));
}
for (let i = 0; i < 3; i++) {
  detected.push(figure8(2, makeFilename(randomLetter10, uniqueLetter)));
}

for (let i = 0; i < 2; i++) {
  detected.push(figure8(3, makeFilename(randomLetter10, uniqueLetter)));
}

detected.push(figure8(5, makeFilename(randomLetter10, uniqueLetter)));
detected.push(simpleCircle(10, makeFilename(randomLetter10, uniqueLetter)));
detected.push(simpleCircle(37, makeFilename(getSameLetter(), uniqueLetter)));
detected.push(
  join(
